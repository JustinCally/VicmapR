...
```{r, echo=FALSE, include=FALSE, warning=FALSE, message=FALSE,eval = all(VicmapR::check_geoserver(quiet = TRUE), !testthat:::on_cran(), sf::sf_extSoftVersion()[["GDAL"]] > 3)}
swoops <- vicmap_query(layer = "ptv_metro_tram_route") %>% collect() 
```
...
vicmap_query(layer = "ptv_metro_tram_route") 
```
...
```{r, warning=FALSE, message=FALSE,eval = all(VicmapR::check_geoserver(quiet = TRUE), !testthat:::on_cran(), sf::sf_extSoftVersion()[["GDAL"]] > 3)}
query <- vicmap_query(layer = "ptv_metro_tram_route") %>% 
  head(10) %>% collect()
...
swoops_2021 <- vicmap_query(layer = "ptv_metro_tram_route") %>% 
  filter(operator_name == "Yarra Trams") %>% 
  filter(num_of_stops > 5) %>% 
  select(id, route_id, route_short_name, trip_headsign, operator_name, route_km) %>%
  collect()
...
swoops_2021 %>%
  ...
  addPolylines(popup = swoops_2021$trip_headsign) #add geom points the marker label (automatically obtained from geom column)
...
swoops_2021 %>%
  ...
  addPolylines(
    ...
    fillOpacity = 0.8,
    popup = swoops_2021$trip_headsign
  )
```
...
# Create a palette
pal <- colorFactor("Accent", levels = swoops_2021$trip_headsign) #define a colour palette
...
swoops_2021 %>%
  ...
  addPolylines(
    ...
    stroke = 0.5, #removes outline
    fillOpacity = 0.8,
    popup = paste0("<b>Route No.: </b>", swoops_2021$route_short_name, "<br>", #format the popup with html tags
                   "<b>Route Name: </b>", swoops_2021$trip_headsign, "<br>",
                   "<b>Distance (km): </b>", swoops_2021$route_km)
  )
```
...
darebin <- vicmap_query(layer = "lga_polygon") %>%
  select(lga_name) %>% 
...
mapview(swoops_en_route, col.regions = "Black") + mapview(route_line)
...
# Swoops in Darebin
darebin_2021 <- vicmap_query(layer = "ptv_metro_tram_route") %>%
...
# Swoops in 10k
radius_2021 <- vicmap_query(layer = "ptv_metro_tram_route") %>%
...
swoop <- vicmap_query(layer = "ptv_metro_tram_route") %>%
  filter(INTERSECTS(route_poly)) %>% 
  collect() %>%
  st_intersection(route_poly)


mapview::mapview(route_poly) + mapview::mapview(swoop, color = "red", lwd = 5)
