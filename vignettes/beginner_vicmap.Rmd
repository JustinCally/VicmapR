...
mapview(darebin_swoops) + mapview(darebin, alpha.regions = 0.3, col.regions= "yellow")
...
mapview(swoops_10k) + mapview(home_radius, alpha.regions = 0.3, col.regions = "yellow")
```
...
mapview(darebin_2021) + mapview(darebin, alpha.regions = 0.3, col.regions = "green")
...
mapview(radius_2021) + mapview(home_radius, alpha.regions = 0.3, col.regions = "green")

```
And finally, the swoops on our cycling route. To find swoops within 300m we use the `DWITHIN()` function.

```{r warning=FALSE, message=FALSE}
# Swoops on route

route_2021 <- vicmap_query(layer = "datavic:FLORAFUANA1_SWOOPING_BIRD") %>%
  filter(SWOOP_DATE > "2020-12-31z") %>%
  filter(STATUS == "OK") %>%
  select(SWOOP_DATE, NO_OF_BIRDS, SPECIES, COMMENTS) %>%
  filter(DWITHIN(route_line, distance = 300)) %>%
  collect()

mapview(route_2021) + mapview(route_line)
