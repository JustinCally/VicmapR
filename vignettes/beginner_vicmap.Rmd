...
The purpose of this tutorial is:
 ...
 - To provide simple examples of downloading and visualising Vicmap spatial data
...
Vicmap is the Victorian Government's catalogue of spatial datasets. The catalogue features `r VicmapR::listLayers() %>% nrow()` datasets across land, property, infrastructure and environment and is the most authoritative suite of spatial data in Victoria. 

### Accessing Vicmap datasets

This data catalogue is freely available to the public and can be accessed via several methods. For users of R, the fastest way to access up to date Vicmap datasets is to utilise the Web Feature Service (WFS). WFS is a standardised interface to request geographic information on the internet, regardless of the platform on which it is stored. 

To use WFS, a URL must be built that describes the desired query or retrieval instructions. This of course requires understanding of the WFS terminology and the time-consuming process of manually building the URL string. 
...
VicmapR allows users to make WFS queries by taking human readable keywords and automatically building the correctly formatted URL string. 
The functions in VicmapR therefore provide a much faster and more robust method for acquiring Vicmap data.

### Lazy evaluation

VicmapR uses *lazy evaluation*, where data can be filtered for a desired subset before downloading. This saves downloading the entire dataset if only a small subset of the data is required. Similarly, the user can filter the first n rows of data before downloading to quickly sample the dataset.
...
Australian spring is famous for the presence of swooping birds. Swooping birds in Victoria include the masked lapwing (plover), butcherbird, magpie-lark, noisy miner and perhaps most famously, the magpie. The Vicmap catalogue has a dataset of 'swoop' locations, recorded by members of the public. 
...
However, if we are still not sure from the description that this is the desired dataset, we can investigate further two more ways. The first way is to execute the Vicmap query, without collecting the data. This is called creating a 'Vicmap Promise'. By viewing the Vicmap promise we get an overview of the dataset as follows.
...
We can see that the dataset contains, swoop date, location and species. At a glance this seems enough to start analysing the spatial distribution of bird swoops. The summary also shows that there are `r nrow(swoops)` entries in this dataset and `r ncol(swoops)` fields. This isn't a very large dataset, however if we are using this data in an interactive report or application, waiting to download the full dataset may be impractical. 

In VicmapR, we can use pipes to filter and subset the data, so that we only collect the desired subset. Let's just look at the first 10 rows.
...
Let's only look at swoops that occurred in 2021, where STATUS is OK. For our analysis, we will look only at the swoop date, species, number of birds, comments and geometry. Note that Vicmap datasets will retain id and OBJECTID columns even if not selected in the Vicmap promise. As this a simple features (sf) dataset, the geometry corresponding to the features (in this case, swoops) does not need to be selected as it is always included. The geometry column is only ever removed by specifically using the `st_drop_geometry()` function in the `sf` package.
...
swoops_2021 %>% head(5) %>% kable()
```

In filtering the Vicmap promise before collecting, we only need to download `r nrow(swoops_2021)` rows of data instead of `r nrow(swoops)`.


### Quick visualisation of spatial data
...
swoops_2021 %>%
  ...
  addCircleMarkers(
    ...
    stroke = FALSE,
    fillOpacity = 0.7,
    popup = paste0("<b>COMMENT: </b>", swoops_2021$COMMENTS, "<br>", #format the popup with html
                   "<b>NUM. BIRDS: </b>", swoops_2021$NO_OF_BIRDS, "<br>",
                   "<b>SPECIES: </b>", swoops_2021$SPECIES)
  )
```

Let's say we want to know about the swoop spots only in our LGA (e.g. Darebin). We can apply a spatial filter to our dataset so that only the swoops in our suburb are shown. Where do we get the spatial data for our LGA you ask? From the Vicmap catalogue of course!
...
To do this we can use `sf::st_intersection()`. Before performing any spatial operations on more than one dataset, you want to make sure the coordinate reference system (crs) is correct and that the crs is the same for each dataset. You can check the crs system with `sf::st_crs()`.

```{r}
# Suburb polygon for Macleod
...
Let's say we feel like a ride. We've picked a route but want to know which areas to avoid so as not to disturb too many nesting birds. This is a more difficult problem as the ride travels through several LGAs and we only really want to know about reported swoops that are very close to the route. 

```{r}
# library(trackeR) #a handy library for handling gps data in R
# route <- readTCX(file = "Mount_Pleasant_Loop_Ride.tcx")

# Load bike route
load("~/Repos/VicmapR/data/route.rda")

# Convert route to single line
route_line <- st_as_sf(route, coords = c("longitude", "latitude"), crs=4326)  %>% #convert coordinates to spatial
  summarise(geometry = st_combine(geometry)) %>% #combine into single line geom
  st_cast("LINESTRING")
