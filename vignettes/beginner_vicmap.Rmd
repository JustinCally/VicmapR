...
VicmapR uses *lazy evaluation*, where data can be filtered before downloading. This saves downloading the entire dataset if only a small subset of the data is required. Similarly, the user can filter the first n rows of data before downloading to quickly sample the dataset.
...
Australian spring is famous for the presence of swooping birds. Some species of birds swoop during the breeding season to protect their nest from potential predators. Swooping birds in Victoria include the masked lapwing (plover), butcherbird, magpie-lark, noisy miner and perhaps most famously, the magpie. The Vicmap catalogue has a dataset of 'swoop' locations, recorded by members of the public. 
...
The summary also shows that there are `r nrow(swoops)` entries in this dataset and `r ncol(swoops)` fields. This isn't a very large dataset, however if we are using this data in an interactive report or application, waiting to download the full dataset may be impractical. 
...
### Quick visualisation of spatial data with Mapview
...
mapview(darebin_swoops) + mapview(darebin, alpha = 0.9, col.regions= "yellow")
...
mapview(swoops_10k) + mapview(home_radius, alpha = 0.9, col.regions = "yellow")
```
