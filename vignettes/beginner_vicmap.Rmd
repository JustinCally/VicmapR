...
The summary also shows that there are `r nrow(swoops)` rows in this dataset. This isn't a very large dataset, but if we are using this data in an interactive report or application, waiting to download the full dataset may be impractical. 
...
**By filtering the Vicmap promise before collecting, we have downloaded only `r nrow(swoops_2021)` rows of data instead of `r nrow(swoops)`.**
...
The VicmapR offers tools for geometric filtering that avoid the need for sf package. The WFS Geoserver on which Vicmap is based supports several geometric filters - see the full list [here](https://justincally.github.io/VicmapR/articles/query_vicmap.html#geometric-filters).
...
Let's revisit our examples above. The first thing to note is the geometric filters available on the geoserver are not as accurate the tools offered in `sf`. To work around this we can use geometric filters first to limit the data for download, then follow up with sf tools. To get the swoops only in Darebin we can filter with `BBOX()`. This filter does not produce a perfect result so we will need to use `st_intersects()` after collecting.
...
darebin_2021 <- vicmap_query(layer = "datavic:FLORAFUANA1_SWOOPING_BIRD") %>% 
  ...
  filter(STATUS == "OK") %>%
  select(SWOOP_DATE, NO_OF_BIRDS, SPECIES, COMMENTS) %>%
  filter(BBOX(darebin, crs = 4328)) %>%
  collect()

mapview(darebin_2021) + mapview(darebin, alpha = 0.9, col.regions = "yellow")

# Tidy up with sf package
darebin_2021_tidy <- st_intersection(darebin_2021, darebin)

mapview(darebin_2021_tidy) + mapview(darebin, alpha = 0.9, col.regions = "yellow")
...
After completing this tutorial you should have a basic undertanding of plotting spatial data and performing some basic spatial operations. This tutorial is a quick guide and designed only to get you started. For further learning, see the following resources:
